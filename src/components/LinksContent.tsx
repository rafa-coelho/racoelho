...
export type LinksSection = 'links' | 'highlights' | 'videos' | 'posts' | 'challenges';

export const DEFAULT_LINKS_SECTION_ORDER: LinksSection[] = [
  'links',
  'highlights',
  'videos',
  'posts',
...
export default function LinksContent({ socialLinks, linkItems, videos = [], posts = [], challenges = [], sectionOrder = DEFAULT_LINKS_SECTION_ORDER }: LinksContentProps) {
  const highlightItems = linkItems.filter(item => item.type === 'highlight');
  const regularLinks = linkItems.filter(item => item.type === 'link');

  const sections: Record<LinksSection, React.ReactNode> = {
    links: regularLinks.length > 0 ? (
      <div key="links" className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12">
    ...
    highlights: highlightItems.length > 0 ? (
      <div key="highlights" className="mb-16">
    ...
    videos: videos.length > 0 ? (
      <div key="videos" className="mb-16">
    ...
    challenges: challenges.length > 0 ? (
      <div key="challenges" className="mb-16">
        <div className="flex justify-between items-center mb-6">
