...
export default function ViewPostPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await pbGetById("posts", id);
        if (!mounted) return;
        setRec(r);

        // Busca views do post - usa cliente público pois listRule=null
        ...
        try {
          // Primeiro tenta buscar TODAS as views para ver se a collection funciona
          const allViews = await pb.collection('post_views').getFullList({
            sort: '-created',
          });
          console.log('[Post Views] Total views in DB:', allViews.length);
          
          const viewsData = await pb.collection('post_views').getFullList({
            filter: `postId="${r.slug}"${dateFilter}`,
            sort: '-created',
          });

          console.log('[Post Views] Loaded views:', viewsData.length, 'for post:', r.slug);
          console.log('[Post Views] First view sample:', viewsData[0]);
          if (mounted) setViews(viewsData as any);
        } catch (error: any) {
          console.error('[Post Views] Error fetching views:', error);
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              ...
              <span>Data: {new Date(rec.date).toLocaleDateString('pt-BR')}</span>
            </div>
          </div>
          <div className="flex gap-2">
            <Link href={`/admin/editor/posts/${id}`}>
              <Button variant="outline" size="sm">Editar Post</Button>
            </Link>
            <a href={`/posts/${rec.slug}`} target="_blank" rel="noopener noreferrer">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {rec.readingTime && (
                  <div>
                    <div className="text-sm text-muted-foreground mb-1">Tempo de leitura</div>
                    <p className="text-sm">{rec.readingTime} min</p>
                  </div>
                )}
              </div>
              {rec.content && (
          ...
          <CardContent className="py-12 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {timeRange !== 'all' 
                ...
                : 'Este post ainda não foi visualizado'}
            </p>
          </CardContent>
        </Card>
      ) : (
