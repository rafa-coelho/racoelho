    ...
    <div className="container mx-auto px-4 py-10">
      <div className="card-modern p-6 max-w-4xl mx-auto space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold">Visualizar Post</h1>
          <div className="flex gap-2">
            <Link href={`/admin/editor/posts/${id}`} className="btn-primary px-3 py-1.5 text-sm">Editar</Link>
            <a href={`/posts/${rec.slug}`} target="_blank" rel="noopener" className="px-3 py-1.5 text-sm border rounded-md hover:bg-white/5">Ver público</a>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          <div className="md:col-span-2">
            ...
            <pre className="whitespace-pre-wrap text-xs bg-white/5 p-3 rounded-md border border-white/10">{rec.content || "—"}</pre>
          </div>
        </div>
      </div>
    </div>
